...
func (s *Server) ActionProcess(_ context.Context, request *server.ActionProcessRequest) (*emptypb.Empty, error) {
	err := s.service.ActionProcess(request.FromChatId, request.Action)
	if err != nil {
		return nil, err
	}
